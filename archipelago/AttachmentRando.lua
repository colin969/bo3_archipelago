...
function generate_weapon_camos_for_seed(seed, base_enabled, base_mixed, pap_enabled, pap_mixed, joined)
    ...
    if base_enabled ~= 0 then
        -- If joined, assume mixed
        local use_mixed = (joined ~= 0) or (base_mixed ~= 0)
        
        local camo_bag
        if use_mixed then
            -- Combine both camo tables
            local combined_camos = {}
            for _, camo in ipairs(camo_table) do
                table.insert(combined_camos, camo)
            end
            for _, camo in ipairs(pap_camo_table) do
                table.insert(combined_camos, camo)
            end
            camo_bag = shuffle_array(combined_camos)
        else
            camo_bag = shuffle_array(camo_table)
        end

        local bag_index = 1
        for _, weapon_data in ipairs(weapon_attachments) do
            local weapon_name = weapon_data.name
            local camo_roll = camo_bag[bag_index]
            
            camo_data[weapon_name] = camo_roll.models.camoIndex
            
            -- If joined and pap enabled, assign same camo to pap'd weapon
            if joined ~= 0 and pap_enabled ~= 0 then
                camo_data[weapon_name .. "_upgraded"] = camo_roll.models.camoIndex
            end
            
            bag_index = bag_index + 1
            if bag_index > #camo_bag then
                bag_index = 1
            end
        end
    end

    if pap_enabled ~= 0 and (joined == 0 or base_enabled == 0) then
        local camo_bag
        if pap_mixed ~= 0 then
            -- Combine both camo tables
            local combined_camos = {}
            ...
            camo_bag = shuffle_array(pap_camo_table)
        end

        local bag_index = 1
        for _, weapon_data in ipairs(weapon_attachments) do
            local weapon_name = weapon_data.name .. "_upgraded"
            local camo_roll = camo_bag[bag_index]
...
function generate_weapon_reticles_for_seed(seed, base_enabled, pap_enabled, joined)
    local numeric_seed = 0
    for i = 1, #seed do
        -- Multi by pos so abc doesn't equal cba
        numeric_seed = numeric_seed + (string.byte(seed, i) * i)
    ...
    if base_enabled ~= 0 then
        local reticle_bag = shuffle_array(reticle_table)

        local bag_index = 1
        for _, weapon_data in ipairs(weapon_attachments) do
            local weapon_name = weapon_data.name
            local reticle_index = reticle_bag[bag_index]
            
            reticle_data[weapon_name] = reticle_index
            
            -- If joined and pap enabled, assign same reticle to pap'd weapon
            if joined ~= 0 and pap_enabled ~= 0 then
                reticle_data[weapon_name .. "_upgraded"] = reticle_index
            end
            
            bag_index = bag_index + 1
            if bag_index > #reticle_bag then
                bag_index = 1
            end
        end
    end

    -- Not joined or base weapon not rando'd, then we rando pap seperate
    if pap_enabled ~= 0 and (joined == 0 or base_enabled == 0) then
        local reticle_bag = shuffle_array(reticle_table)
...
function load_camos_into_gsc(camo_data)
    for weapon_name, camoIndex in pairs(camo_data) do
        Engine.SetDvar("ARCHIPELAGO_CAMO_RANDO_" .. string.upper(weapon_name), camoIndex)
    end
end

function load_reticles_into_gsc(reticle_data)
    for weapon_name, reticle in pairs(reticle_data) do
        Engine.SetDvar("ARCHIPELAGO_RETICLE_RANDO_" .. string.upper(weapon_name), reticle)
    end
end

return {
    weapon_attachments = weapon_attachments,
    generate_weapon_camos_for_seed = generate_weapon_camos_for_seed,
    generate_weapon_attachments_for_seed = generate_weapon_attachments_for_seed,
    generate_weapon_reticles_for_seed = generate_weapon_reticles_for_seed,
    load_attachments_into_gsc = load_attachments_into_gsc,
    load_camos_into_gsc = load_camos_into_gsc,
    load_reticles_into_gsc = load_reticles_into_gsc,
}
