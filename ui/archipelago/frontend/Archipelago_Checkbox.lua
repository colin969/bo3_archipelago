...
local PostLoadFunc = function ( element, controller, menu )
    element:registerEventHandler( "options_refresh", function ( element, event )
        element:processEvent( {
            name = "update_state",
            controller = event.controller
        } )
    end )
    menu:AddButtonCallbackFunction( element, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
        if not menu.m_disableNavigation and element:AcceptGamePadButtonInputFromModelCallback( controller ) and type( element.checkboxAction ) == "function" then
            element.checkboxAction( controller, element )
            element:processEvent( {
                name = "update_state",
...
-- Simple function to check if checkbox is checked based on model value (0 or 1)
local function SimpleCheckBoxChecked( element, event )
    local elementModel = element:getModel()
    if elementModel then
        local checkedModel = Engine.GetModel( elementModel, "checked" )
        if checkedModel then
            return Engine.GetModelValue( checkedModel ) == 1
        end
    end
    return false
end

CoD.Archipelago_CheckBox = InheritFrom( LUI.UIElement )
CoD.Archipelago_CheckBox.new = function ( menu, controller )
    local self = LUI.UIElement.new()
    if PreLoadFunc then
        PreLoadFunc( self, controller )
    end
    self:setUseStencil( false )
    self:setClass( CoD.Archipelago_CheckBox )
    self.id = "Archipelago_CheckBox"
    ...
    -- Use simple model-based check (0 = unchecked, 1 = checked)
    self:mergeStateConditions( {
        {
            stateName = "Checked",
            condition = function ( menu, element, event )
                return SimpleCheckBoxChecked( element, event )
            end
        }
    } )
    
    -- Subscribe to model changes to update state
