...
function private pap_trigger_hintstring_monitor()
{
	...
	// Wait until 1 progressive to start monitor
	wait_till_owned();
	...
	while( true )
	{
		foreach( e_player in level.players )
		{
			if ( e_player istouching( self ) )
		    {
				...
				w_curr_player_weapon = e_player getcurrentweapon();
				if(zm_weapons::is_weapon_upgraded(w_curr_player_weapon))
				{
					// Require 2 progressives to use AATs
					item = level.archi.items["Progressive - Pack-A-Punch Machine"];
					if (isdefined(item) && item.count >= 2)
					{
						self sethintstring(&"ZOMBIE_PERK_PACKAPUNCH_AAT", self.aat_cost);
					}
					else
					{
						self SetHintString("At least 2x 'Progressive - Pack-A-Punch Machine' is required");
					}
				}
				else
				{
...
function private vending_weapon_upgrade()
{
	...
	self SetHintString("'Progressive - Pack-A-Punch Machine' is required");
...
function wait_till_owned()
{
	while (true)
	{
		level waittill("Pack_A_Punch_on");
		if (isdefined(level.archi) && isdefined(level.archi.items))
		{
			item = level.archi.items["Progressive - Pack-A-Punch Machine"];
