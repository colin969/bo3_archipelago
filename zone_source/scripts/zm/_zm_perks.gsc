...
{
   	level.perk_mappings = [];
    level.perk_mappings[PERK_JUGGERNOG] = ARCHIPELAGO_ITEM_PERK_JUGGERNOG;
    level.perk_mappings[PERK_QUICK_REVIVE] = ARCHIPELAGO_ITEM_PERK_QUICK_REVIVE;
    level.perk_mappings[PERK_SLEIGHT_OF_HAND] = ARCHIPELAGO_ITEM_PERK_SLEIGHT_OF_HAND;
    level.perk_mappings[PERK_DOUBLETAP2] = ARCHIPELAGO_ITEM_PERK_DOUBLETAP2;
    level.perk_mappings[PERK_STAMINUP] = ARCHIPELAGO_ITEM_PERK_STAMINUP;
    level.perk_mappings[PERK_PHDFLOPPER] = ARCHIPELAGO_ITEM_PERK_PHDFLOPPER;
    level.perk_mappings[PERK_DEAD_SHOT] = ARCHIPELAGO_ITEM_PERK_DEAD_SHOT;
    level.perk_mappings[PERK_ADDITIONAL_PRIMARY_WEAPON] = ARCHIPELAGO_ITEM_PERK_ADDITIONAL_PRIMARY_WEAPON;
    level.perk_mappings[PERK_ELECTRIC_CHERRY] = ARCHIPELAGO_ITEM_PERK_ELECTRIC_CHERRY;
    level.perk_mappings[PERK_TOMBSTONE] = ARCHIPELAGO_ITEM_PERK_TOMBSTONE;
    level.perk_mappings[PERK_WHOSWHO] = ARCHIPELAGO_ITEM_PERK_WHOSWHO;
    level.perk_mappings[PERK_VULTUREAID] = ARCHIPELAGO_ITEM_PERK_VULTUREAID;
    level.perk_mappings[PERK_WIDOWS_WINE] = ARCHIPELAGO_ITEM_PERK_WIDOWS_WINE;
	...
	if ( level._custom_perks.size > 0 )
	{
		...
		for ( i = 0; i < a_keys.size; i++ )
		{
			...
			if( isdefined( level._custom_perks[ a_keys[ i ] ].perk_machine_power_override_thread ) && a_keys[i] != "specialty_quickrevive" ) // Ignore quick revive solo power logic
			{
...
{	
	...
	while ( true )
	{
		...
		wait_till_owned(str_key, str_on);
...
function set_ap_hintstring(perk_str)
{
	while(true)
	{
		if (isdefined(level.archi) && isdefined(level.archi.map_specific_machines) && isdefined(level.archi.mapString))
		{
			mapping = level.perk_mappings[perk_str];
			if (level.archi.map_specific_machines == 1)
			{
				if (isdefined(mapping))
				{
...
function wait_till_owned(perk_str, notify_name)
{
	while (true)
	{
		if (isdefined(level.archi) && isdefined(level.archi.active_perk_machines))
		{
			perk_active = isdefined(level.archi.active_perk_machines[perk_str]) && level.archi.active_perk_machines[perk_str] == true;
			if (perk_active)
			{
				level thread sync_on_delay();
				break;
			}
		}
...
function sync_on_delay()
{
	wait(0.2);
	if (isdefined(level.archi.sync_perk_exploders))
	{
		[[level.archi.sync_perk_exploders]]();
	}
}
...
function vending_trigger_think()
{
	...
	self thread set_ap_hintstring(perk);
	...
	wait_till_owned(perk, notify_name);
	//level waittill( notify_name );
	start_on = false;
...
function check_player_has_perk(perk)
{
	...
	while(true)
	{
		...
		for( i = 0; i < players.size; i++ )
		{
			...
			{
				if(players[i] zm_utility::can_player_purchase_perk() && 
				   !players[i] HasPerk(perk) && 
