...
function give_Trap_KnuckleCrack()
{
    foreach (player in level.players)
    {
        if (zm_utility::is_player_valid(player) && IsAlive(player))
        {
...
function _drop_powerup(powerup)
{
    ...
    if (players.size > 0) 
    {
        ...
        powerup_drop = level zm_powerups::specific_powerup_drop(powerup, player.origin);
    }
}

function _drop_powerup_with_delay(powerup, delay)
{
    players = GetPlayers();
    if (players.size > 0) 
    {
        player = array::random(players);
        powerup_drop = level zm_powerups::specific_powerup_drop(powerup, player.origin, undefined, undefined, delay);
    }
}
